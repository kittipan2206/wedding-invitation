...
function initEventTab() {
  ...
  document.getElementById('btn-save-event')?.addEventListener('click', async () => {
    ...
    if (hasChange) { markEvClean(); showToast('บันทึก Event Info แล้ว ✓'); }
...
async function loadPhotos() {
  ...
  try {
    ...
    photos = Array.isArray(data)
      ? data.filter(p => p && typeof p.url === 'string' && p.url.startsWith('http'))
      : [];
  } catch { photos = []; }
  renderPhotoList();
  updatePhotoCount();
...
function renderPhotoList() {
  ...
  if (!list) return;
  if (photos.length === 0) {
    list.innerHTML = '<p class="admin-status">ยังไม่มีรูปภาพ — เพิ่มรูปแรกได้เลย</p>';
    return;
  }
  list.innerHTML = '';
...
function buildPhotoItem(photo, index) {
  ...
  const catLabel = cat === 'pre-wedding' ? 'Pre-wedding' : cat === 'wedding' ? 'วันงาน' : cat || '—';
  ...
  item.querySelector('.btn-icon--vis').addEventListener('click', () => handleToggleVisible(photo.id, !photo.visible));
  item.querySelector('.btn-icon--up').addEventListener('click',  () => handleReorder(index, index - 1));
  item.querySelector('.btn-icon--down').addEventListener('click',() => handleReorder(index, index + 1));
  item.querySelector('.btn-icon--del').addEventListener('click', () => handleDeletePhoto(photo.id));
  return item;
}
...
function handleToggleVisible(id, visible) {
  ...
  renderPhotoList(); updatePhotoCount();
...
function handleReorder(fromIdx, toIdx) {
  ...
  const a = photos[fromIdx], b = photos[toIdx];
...
function renderRsvp() {
  ...
  const coming    = rsvpData.filter(r => String(r['การเข้าร่วม']).includes('ยินดี'));
  const notcoming = rsvpData.filter(r => String(r['การเข้าร่วม']).includes('ไม่สะดวก'));
  const guests    = coming.reduce((s, r) => s + (Number(r['จำนวน']) || 0), 0);
  document.getElementById('rsvp-count-coming').textContent    = coming.length;
  document.getElementById('rsvp-count-notcoming').textContent = notcoming.length;
  ...
  tbody.innerHTML = filtered.map((r, i) => {
    ...
    const badgeClass = isComing ? 'status-badge--coming' : 'status-badge--notcoming';
...
function exportRsvpCsv() {
  if (!rsvpData.length) { showToast('ไม่มีข้อมูล', 'error'); return; }
  const headers = ['#','ชื่อ','การเข้าร่วม','จำนวน','ติดต่อ','ข้อความ','เวลา'];
...
function initRsvpTab() {
  ...
  document.getElementById('rsvp-filter')?.addEventListener('change', renderRsvp);
  document.getElementById('btn-export-rsvp')?.addEventListener('click', exportRsvpCsv);
}
...
function initGuestbookTab() {
  document.getElementById('show-hidden-entries')?.addEventListener('change', renderGuestbook);
}
...
function initMusicTab() {
  ...
  ['music-url'].forEach(id => {
    document.getElementById(id)?.addEventListener('input', () => markUnsaved('music-unsaved'));
  });
  ['travel-airport','travel-hotel','travel-car'].forEach(id => {
    document.getElementById(id)?.addEventListener('input', () => markUnsaved('travel-unsaved'));
  });
  ...
  document.getElementById('btn-save-travel')?.addEventListener('click', async () => {
    const btn = document.getElementById('btn-save-travel');
    btn.disabled = true; btn.textContent = 'กำลังบันทึก…';
    const fields = {
      travel_airport: document.getElementById('travel-airport')?.value.trim() || '',
      travel_hotel:   document.getElementById('travel-hotel')?.value.trim()   || '',
      travel_car:     document.getElementById('travel-car')?.value.trim()     || '',
    };
    await Promise.allSettled(
      Object.entries(fields).map(([key, value]) => apiPost({ type: 'config_update', key, value }))
    );
