...
function normalizeConfigValues(data) {
  ...
  Object.entries(data).forEach(([key, raw]) => {
    if (typeof raw !== 'string' || !isoRe.test(raw)) { out[key] = raw; return; }
    const d = new Date(raw);
    if (isNaN(d.getTime())) { out[key] = raw; return; }
...
export function injectConfig(cfg) {
  ...
  if (heroNames) {
    heroNames.innerHTML =
      `${c.groom_name} <span class="ampersand">&amp;</span> ${c.bride_name}`;
  }

  // Footer names + date
  const footerNames = document.getElementById('footer-names');
  if (footerNames) footerNames.textContent = `${c.groom_name} & ${c.bride_name}`;
