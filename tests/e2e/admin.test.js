...
const GALLERY_PHOTOS = [
  { id: "photo-1", url: "https://picsum.photos/seed/g1/400/300", caption: "รูปแรก", category: "wedding", visible: true, order: 1 },
  { id: "photo-2", url: "https://picsum.photos/seed/g2/400/300", caption: "รูปที่สอง", category: "pre-wedding", visible: false, order: 2 },
];
...
test.describe("Admin — gallery tab", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page.locator("#tab-gallery")).not.toHaveClass(/tab-panel--hidden/, { timeout: 5_000 });
  });

  test("existing photos are listed with thumbnails and captions", async ({ page }) => {
    await expect(page.locator("#photo-list .photo-item")).toHaveCount(2, { timeout: 8_000 });
    await expect(page.locator(".photo-caption-input").first()).toHaveValue("รูปแรก");
    await expect(page.locator(".photo-caption-input").last()).toHaveValue("รูปที่สอง");
  });

  test("hidden photo has hidden indicator", async ({ page }) => {
    await expect(page.locator("#photo-list .photo-item--hidden")).toHaveCount(1, { timeout: 8_000 });
  });

  test("add photo form is present", async ({ page }) => {
    await expect(page.locator("#add-photo-form")).toBeVisible({ timeout: 5_000 });
  });
  ...
  test("adding a photo by URL appears in the list", async ({ page }) => {
    ...
    await expect(page.locator("#photo-list .photo-item")).toHaveCount(3, { timeout: 5_000 });
  });

  test("caption input is editable inline", async ({ page }) => {
    await expect(page.locator(".photo-caption-input").first()).toBeVisible({ timeout: 8_000 });
    await page.locator(".photo-caption-input").first().fill("แก้ไขแล้ว");
    await expect(page.locator(".photo-caption-input").first()).toHaveValue("แก้ไขแล้ว");
  });

  test("toggle visibility button is present per photo", async ({ page }) => {
    await expect(page.locator(".btn-icon--vis").first()).toBeVisible({ timeout: 8_000 });
  });

  test("clicking toggle visibility updates the photo item class", async ({ page }) => {
    await expect(page.locator("#photo-list .photo-item")).toHaveCount(2, { timeout: 8_000 });
    // First photo is visible — clicking hide toggles it to hidden
    await page.locator(".btn-icon--vis").first().click();
    await expect(page.locator("#photo-list .photo-item--hidden")).toHaveCount(2, { timeout: 3_000 });
  });

  test("reorder up/down buttons are present", async ({ page }) => {
    await expect(page.locator(".btn-icon--up").first()).toBeVisible({ timeout: 8_000 });
    await expect(page.locator(".btn-icon--down").first()).toBeVisible({ timeout: 8_000 });
  });
  ...
  test("empty state shown when no photos exist", async ({ page }) => {
    ...
    await page.evaluate(() => { /* reset tabLoaded via reload workaround */ });
    await page.goto("/admin.html");
    await page.evaluate(() => sessionStorage.setItem("gallery_admin_auth", "1"));
    await mockGAS(page, { photos: [] });
    await page.reload();
    await mockGAS(page, { photos: [] });
    await page.goto("/admin.html");
    ...
    await expect(page.locator("#photo-list")).toContainText(/ไม่มีรูปภาพ|เพิ่มรูปแรก/, { timeout: 8_000 });
  });
});
