...
test.describe("RSVP — happy path", () => {
  test("submitting mails the reply and shows a personalized thank-you with calendar buttons", async ({
    page,
  }) => {
    await mockGAS(page);
    ...
    // The mail-away envelope animation plays (~4s), then the thank-you
    await expect(page.locator("#thank-you")).toBeVisible({ timeout: 8_000 });
    await expect(page.locator("#rsvp-form")).not.toBeVisible();
    // Personalized greeting + the attend variant offers both calendar buttons
    await expect(page.locator("#ty-heading")).toContainText("คุณสมชาย");
    await expect(page.locator("#ty-calendar")).toBeVisible();
    await expect(page.locator("#ty-gcal-btn")).toHaveAttribute(
      "href",
      /calendar\.google\.com/,
    );
    await expect(page.locator("#ty-ics-btn")).toBeVisible();
  });

  test("declining shows a warm thank-you without calendar buttons", async ({
    page,
  }) => {
    await mockGAS(page);
    await page.goto("/?goto=rsvp");
    ...
    await page.fill("#guest-name", "สมปอง");
    await page.selectOption("#guest-count", "1");
    await page.click('label[for="attend-no"]');
    await page.click('button[type="submit"]');

    await expect(page.locator("#thank-you")).toBeVisible({ timeout: 8_000 });
    await expect(page.locator("#ty-heading")).toContainText("คุณสมปอง");
    await expect(page.locator("#ty-calendar")).toBeHidden();
  });

  test("personalized thank-you persists after reload", async ({ page }) => {
    await mockGAS(page);
    await page.goto("/?goto=rsvp");
    await expect(page.locator("#rsvp-form")).toBeVisible({ timeout: 10_000 });
    ...
    await expect(page.locator("#rsvp-form")).not.toBeVisible();
    // No animation on reload — straight to the same personalized state
    await expect(page.locator("#ty-heading")).toContainText("คุณสมหญิง");
    await expect(page.locator("#ty-calendar")).toBeVisible();
  });
});
...
test.describe("RSVP — validation", () => {
  ...
  test("character counter updates as user types in note field", async ({
    page,
  }) => {
...
test.describe("RSVP — deadline", () => {
  test("shows closed banner when past RSVP deadline", async ({ page }) => {
    await mockGAS(page, {
      config: {
        rsvp_deadline_iso: "2020-01-01",
        rsvp_deadline_display: "1 มกราคม 2563",
      },
    });
    await page.goto("/?goto=rsvp");
    await expect(page.locator("#rsvp-closed-banner")).toBeVisible({
      timeout: 10_000,
    });
  });
});
...
test.describe("RSVP — edge cases", () => {
  test("prevents double submission — button disabled after first click", async ({
    page,
  }) => {
  ...
  test("Thai and special characters in name are handled correctly", async ({
    page,
  }) => {
    await mockGAS(page);
    await page.goto("/?goto=rsvp");
